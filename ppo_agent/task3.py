...
def combat_bfs_oc9(game_state):
    ...
    if distance is None:
        ...
        targets.update(others) 
        
        direction, distance = bfs_first_step(field, origin, list(targets), bombs, others)
...
def task3_action_mask(game_state):
    """Keep moves with a timed escape; bomb for safely reachable crate OR opponent damage."""
    field = game_state['field']
    origin = tuple(game_state['self'][3])
    ...
    if can_place_bomb(origin, game_state['self'][2], occupied):
        crates, opponents_hit = bomb_effects_from(origin, field, others)
        

        mask[5] = (crates > 0 or opponents_hit > 0) and has_safe_bomb_escape(
